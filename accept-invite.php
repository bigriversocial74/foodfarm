...
if (preg_match('/^[a-f0-9]{64}$/', $token)) {
    $lookup = $pdo->prepare("SELECT id, household_id, email, display_name, age_group, role, permission_overrides FROM household_invitations WHERE token_hash = ? AND accepted_at IS NULL AND revoked_at IS NULL AND expires_at > UTC_TIMESTAMP() LIMIT 1");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && is_array($invitation)) {
    try {
        ...
        $lock = $pdo->prepare("SELECT id, household_id, email, display_name, age_group, role, permission_overrides FROM household_invitations WHERE id = ? AND token_hash = ? AND accepted_at IS NULL AND revoked_at IS NULL AND expires_at > UTC_TIMESTAMP() FOR UPDATE");
        ...
        $createUser->execute([strtolower((string)$invitation['email']), password_hash($password, PASSWORD_DEFAULT), $invitation['display_name']]);
        ...
        $consume = $pdo->prepare('UPDATE household_invitations SET accepted_at = UTC_TIMESTAMP() WHERE id = ? AND accepted_at IS NULL AND revoked_at IS NULL');
...
<?php if ($error): ?><div class="status status-warning" style="display:block;margin:20px 0"><?= e($error) ?></div><?php elseif (is_array($invitation)): ?><p class="page-description">Create an account for <strong><?= e((string)$invitation['email']) ?></strong> as <?= e(str_replace('_', ' ', (string)$invitation['role'])) ?>.</p><form method="post" class="form-grid"><input type="hidden" name="csrf_token" value="<?= e(csrf_token()) ?>"><input type="hidden" name="token" value="<?= e($token) ?>"><label>Password<input class="search-field" type="password" name="password" minlength="12" autocomplete="new-password" required></label><label>Confirm password<input class="search-field" type="password" name="password_confirmation" minlength="12" autocomplete="new-password" required></label><button class="button primary" type="submit">Create account</button></form><?php endif; ?></section></main></body></html>
