...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        match ($action) {
            ...
            'attach_recipe' => $service->attachRecipe((int)($_POST['starter_kit_version_id'] ?? 0), (int)($_POST['recipe_id'] ?? 0)),
...
$items = $pdo->query("SELECT i.*, v.sku, v.status AS version_status, k.name AS kit_name FROM starter_kit_items i JOIN starter_kit_versions v ON v.id=i.starter_kit_version_id JOIN starter_kits k ON k.id=v.starter_kit_id ORDER BY i.id DESC LIMIT 100")->fetchAll();
...
$recipes = $pdo->query("SELECT id,name FROM recipes WHERE status='active' ORDER BY name LIMIT 250")->fetchAll();
?><!doctype html>
...
<header class="page-header"><div><p class="eyebrow">Platform administration</p><h1>Starter Kits</h1><p class="page-description">Define basic and specialized kits, freeze published versions, and provision pantry, shopping, recipes, and household tasks.</p></div><div><a class="button secondary" href="/phase4.php">Recipes</a> <a class="button secondary" href="/phase3.php">Access</a></div></header>
...
<article class="panel"><h2>Create kit</h2><form method="post" class="form-grid"><input type="hidden" name="csrf_token" value="<?= e(csrf_token()) ?>"><input type="hidden" name="action" value="create_kit"><label>Name<input class="search-field" name="name" required></label><label>Slug<input class="search-field" name="slug" pattern="[a-z0-9]+(?:-[a-z0-9]+)*" required></label><label>Type<select name="kit_type"><option value="basic">Basic</option><option value="specialized">Specialized</option></select></label><label>Category<input class="search-field" name="category"></label><label>Status<select name="status"><option value="draft">Draft</option><option value="published">Published</option></select></label><label>Description<textarea name="description"></textarea></label><button class="button primary">Create kit</button></form></article>

<article class="panel"><h2>Create version</h2><form method="post" class="form-grid"><input type="hidden" name="csrf_token" value="<?= e(csrf_token()) ?>"><input type="hidden" name="action" value="create_version"><label>Kit<select name="starter_kit_id" required><?php foreach($kits as $kit): if($kit['status']==='retired') continue; ?><option value="<?= (int)$kit['id'] ?>"><?= e((string)$kit['name']) ?></option><?php endforeach; ?></select></label><label>Version<input class="search-field" type="number" min="1" name="version_number" required></label><label>SKU<input class="search-field" name="sku" required></label><label>Price<input class="search-field" type="number" min="0" step="0.01" name="price"></label><label>Status<select name="status"><option value="draft">Draft</option><option value="published">Published</option></select></label><button class="button primary">Create version</button></form></article>

<article class="panel span-2"><h2>Add item to draft version</h2><form method="post" class="form-grid"><input type="hidden" name="csrf_token" value="<?= e(csrf_token()) ?>"><input type="hidden" name="action" value="add_item"><label>Draft version<select name="starter_kit_version_id" required><?php foreach($draftVersions as $version): ?><option value="<?= (int)$version['id'] ?>"><?= e((string)$version['kit_name']) ?> · <?= e((string)$version['sku']) ?></option><?php endforeach; ?></select></label><label>Item name<input class="search-field" name="item_name" required></label><label>Kind<select name="item_kind"><option>ingredient</option><option>equipment</option><option>supply</option><option>seed</option><option>digital</option></select></label><label>Fulfillment<select name="fulfillment_type"><option value="shopping_list">Shopping list</option><option value="shipped">Shipped</option><option value="optional_delivery">Optional delivery</option><option value="digital_only">Digital only</option><option value="customer_supplied">Customer supplied</option></select></label><label>Quantity<input class="search-field" type="number" step="0.0001" min="0" name="default_quantity"></label><label>Unit<input class="search-field" name="unit"></label><label>Inventory category<select name="inventory_category_id"><option value="">None</option><?php foreach($categories as $category): ?><option value="<?= (int)$category['id'] ?>"><?= e((string)$category['name']) ?></option><?php endforeach; ?></select></label><label>Suggested storage<input class="search-field" name="suggested_storage_type"></label><label>Reorder level<input class="search-field" type="number" step="0.0001" min="0" name="reorder_level"></label><label>Estimated price<input class="search-field" type="number" step="0.01" min="0" name="estimated_price"></label><label>Supplier<input class="search-field" name="supplier_name"></label><label><input type="checkbox" name="required" value="1" checked> Required</label><label><input type="checkbox" name="shipping_eligible" value="1"> Shipping eligible</label><label><input type="checkbox" name="delivery_eligible" value="1"> Delivery eligible</label><button class="button primary">Add item</button></form></article>

<article class="panel"><h2>Attach starter recipe</h2><form method="post" class="form-grid"><input type="hidden" name="csrf_token" value="<?= e(csrf_token()) ?>"><input type="hidden" name="action" value="attach_recipe"><label>Draft version<select name="starter_kit_version_id" required><?php foreach($draftVersions as $version): ?><option value="<?= (int)$version['id'] ?>"><?= e((string)$version['kit_name']) ?> · <?= e((string)$version['sku']) ?></option><?php endforeach; ?></select></label><label>Recipe<select name="recipe_id" required><?php foreach($recipes as $recipe): ?><option value="<?= (int)$recipe['id'] ?>"><?= e((string)$recipe['name']) ?></option><?php endforeach; ?></select></label><button class="button primary">Attach recipe</button></form></article>

<article class="panel"><h2>Add starter task</h2><form method="post" class="form-grid"><input type="hidden" name="csrf_token" value="<?= e(csrf_token()) ?>"><input type="hidden" name="action" value="add_task"><label>Draft version<select name="starter_kit_version_id" required><?php foreach($draftVersions as $version): ?><option value="<?= (int)$version['id'] ?>"><?= e((string)$version['kit_name']) ?> · <?= e((string)$version['sku']) ?></option><?php endforeach; ?></select></label><label>Task title<input class="search-field" name="title" required></label><label>Area<input class="search-field" name="area"></label><label>Due after days<input class="search-field" type="number" name="due_offset_days" value="0"></label><label>Recurrence rule<input class="search-field" name="recurring_rule"></label><label>Instructions<textarea name="instructions"></textarea></label><button class="button primary">Add task</button></form></article>
