...
login_page="$(mktemp)"
logout_page="$(mktemp)"
trap 'rm -f "$cookie_jar" "$login_page" "$logout_page"' EXIT
...
[[ "$(status "$base_url/index.php")" == "200" ]] || fail "public shell unavailable"
pass "public shell responds"

[[ "$(status "$base_url/login.php")" == "200" ]] || fail "login unavailable"
pass "login page responds"

[[ "$(status "$base_url/phase2.php")" == "303" ]] || fail "protected Phase 2 did not redirect"
pass "protected route redirects unauthenticated requests"
...
for route in phase2.php phase3.php phase4.php phase5.php account.php; do
